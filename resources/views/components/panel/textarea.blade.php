...
<div class="space-y-1.5">
    ...
    <textarea name="{{ $name }}" id="{{ $name }}" rows="{{ $rows }}" @if($required) required @endif
        {{ $attributes->merge(['class' => 'w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-3 text-on-surface focus:border-gold-light focus:ring-1 focus:ring-gold-light focus:outline-none transition-colors']) }}>{{ old($name, $value) }}</textarea>
